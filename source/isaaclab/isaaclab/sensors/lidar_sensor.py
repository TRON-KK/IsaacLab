...
from .ray_caster import MultiMeshRayCaster
...
class LidarSensor(MultiMeshRayCaster):
    """A LiDAR sensor implementation based on multi-mesh ray-casting.

    Extends :class:`MultiMeshRayCaster` so the LiDAR can ray-cast against multiple,
    independently-moving meshes (rigid bodies, articulated links) in addition to the
    static environment. All LiDAR-specific behaviour — Livox / grid patterns, dynamic
    pattern rotation, noise, dropout, point-cloud generation — is kept unchanged.
    """
    ...
    def __str__(self) -> str:
        ...
        # `_num_meshes_per_env` is populated by MultiMeshRayCaster after _initialize_warp_meshes.
        meshes_per_env = sum(self._num_meshes_per_env.values()) if hasattr(self, "_num_meshes_per_env") else 0
        return (
            f"LiDAR Sensor @ '{self.cfg.prim_path}': \n"
            f"\tview type            : {self._view.__class__}\n"
            f"\tupdate period (s)    : {self.cfg.update_period}\n"
            f"\tupdate frequency (Hz): {self.cfg.update_frequency}\n"
            f"\tnumber of meshes     : {self._num_envs} x {meshes_per_env}\n"
