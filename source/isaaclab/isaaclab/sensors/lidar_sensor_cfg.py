...
from .ray_caster import MultiMeshRayCasterCfg
from .lidar_sensor import LidarSensor


@configclass
class LidarSensorCfg(MultiMeshRayCasterCfg):
    """Configuration for the LiDAR sensor.

    Extends :class:`MultiMeshRayCasterCfg` to add LiDAR-specific parameters (noise
    simulation, point cloud generation, sensor-specific settings) while inheriting the
    multi-mesh capabilities: ``mesh_prim_paths`` accepts both plain strings (treated as
    static targets) and :class:`MultiMeshRayCasterCfg.RaycastTargetCfg` entries with
    ``track_mesh_transforms=True`` for dynamic targets, ``update_mesh_ids`` returns the
    per-ray mesh id, and ``reference_meshes`` enables shared-mesh deduplication.
    """
