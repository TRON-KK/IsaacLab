...
from .ray_caster import MultiMeshRayCasterData
...
@dataclass
class LidarSensorData(MultiMeshRayCasterData):
    """Data container for the LiDAR sensor.

    Extends :class:`MultiMeshRayCasterData` so consumers get both the multi-mesh fields
    (``ray_mesh_ids``) and the LiDAR-specific fields (``distances``, ``pointcloud``).
    """
